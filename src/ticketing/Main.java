...
final public ImageIcon icon = (new ImageIcon(getClass().getResource("ProgramBanner.png")));
...
//STARTOF ACTIONEVENTS
	...
	public void actionPerformed (ActionEvent action)	// Method that contain all conditions where an ActionEvent is needed
		...
		if (action.getSource() == timeCombo)
		{
			...
			ArrayList<Integer> timeArray  = FullDataBaseGenerator.AvailableSeatsArrayReturn(selectedTime);
...
//DRAW LEFT BLOCK OF SEATS
		...
		{
			String colString = Integer.toString(i+101);								// Creates an Integer of relative Seat Number and converts it to a String
			...
			{
				String rowString = Integer.toString((i+(leftBlockSeatsCol*x))+101);			// [As above]
...
// DRAW CENTER BLOCK OF SEATS
		...
		{
			String colString = Integer.toString(i+201);
...
//DRAW RIGHT BLOCK OF SEATS
		...
		{
			String colString = Integer.toString(i+301);
			...
			{
				String rowString = Integer.toString((i+(rightBlockSeatsCol*x))+301);
...
/*
	...
	{
		launch(args);							// Create a new Instance of the main Program when ran
	}
  */       
}
